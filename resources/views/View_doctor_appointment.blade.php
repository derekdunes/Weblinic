...
@section('content2')
 <div class="table-responsive">
    ...
    @foreach($appoint as $value)
     ...
     <tr>
     <td>{{$value->appointment_id}}</td><td>{{$value->start_time}}</td><td>{{$value->end_time}}</td><td>{{ $value->patientfirstName . ' ' . $value->patientlastName }}</td><td> <a href='/treatment/{{ $value->appointment_id}}'> Treatment</a></td>
